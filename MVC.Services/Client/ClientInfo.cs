	...
	public class ClientInfo
	{
		...
		public string RenderingEngineVersion
		{
			get
			{
				...
				var match = Regex.Match(this.userAgent, @"(WebKit|Trident|Presto)/(?<version>\d+(\.\d+)*)");
		...
		public string OS
		{
			get
			{
				...
				{
                    // Windows Desktop
                    if (this.userAgent.Contains("Windows NT 10")) return "Windows 10";
				...
				{
					var match = Regex.Match(this.userAgent, @"Android \d+\.\d+");
                    return match.Success ? match.Value : "Android";
				}
                else if (this.userAgent.Contains("Linux"))
                {
                    var match = Regex.Match(this.userAgent, @"Ubuntu/\d+");
                    return match.Success ? match.Value.Replace('/', ' ') : "Linux";
                }
